...
Процедура ПроверкаКорректностиПериода()  
	...
	Если ЗначениеЗаполнено(Объект.КонецПериода) И Объект.НачалоПериода >  Объект.КонецПериода Тогда
		...
		Сообщение.Текст = "Начало периода не может быть больше конца!";
		Сообщение.Сообщить();
...
Процедура ЗагрузитьВГрафикиПредприятия(Команда)
	...
	Если (Объект.СписокДней.Количество() > 0) И НЕ (Объект.Ссылка.Пустая()) Тогда
		ЗагрузитьВГрафикиПредприятияНаСервере();
	Иначе
		Сообщить("График работы не содержит данных, сначала заполните график работы и запишите в базу!");
	КонецЕсли; 
